...
# PROMPT-REBUILD-V2
## 글로벌 뉴스 + 실시간 시장데이터 + AI 주식비서 재구축 (배포오류/썸네일/실API 연동 포함)

---

## 0. 프로젝트 목적 (반드시 준수)

현재 구현된 글로벌 뉴스 대시보드 + 주식 시그널/전략 패널 시스템은 화면은 동작하나, 실제 운영 기준에서는 다음 문제가 존재한다.

- 배포 후 검증이 불충분하여 실제 기능 정상 동작 여부를 신뢰할 수 없음
- 뉴스 카드 썸네일이 이미지가 아닌 텍스트/플레이스홀더로 표시됨
- 실시간 주식/자원 데이터가 아닌 Mock/가짜 데이터가 표시될 가능성이 높음
- 전략 패널에 동일 종목 반복 노출, 핵심분야(자원/방산/우주/AI/반도체/로봇/에너지) 반영 미흡
- 상세보기 분석이 무의미한 50:50 확률 수치에 수렴
- 뉴스-종목 매핑이 약하여 종목별 호재/악재 분석 품질이 낮음
- 리스크/자금관리 패널의 정보가 실사용 판단에 도움이 되지 않음

### 최종 목표
1. **Toss MCP 또는 키움 REST API 기반 실데이터 수집**
2. **뉴스 + 시세 + 거래량 + 이벤트 기반 종목 분석 고도화**
3. **RAG 기반 AI 주식비서(경량 모델)**
4. **수동 매매 / 관리자 승인 기반 자동화 확장 준비**
5. **기존 뉴스 포털 기능 유지 + 운영 가능한 관리자 진단 체계 확보**

---

## 1. 필수 전제 및 공통 규칙 (Codex 작업 전 반드시 읽고 준수)

### 저장소 / 브랜치 / 문서
- GitHub Repository: `https://github.com/wadiworks3802-kr`
- Codex는 해당 GitHub 계정(구글 연동 계정)으로 이미 로그인되어 있음
- 기준 문서:
  - `prompt.md` (기존 요구사항/작업 프롬프트)
  - `work_timeline.md` (지금까지 작업 이력)
  - 본 문서 `PROMPT-REBUILD-V2`

### 공통 원칙
1. 기존 글로벌 뉴스 포털 기능은 깨지지 않게 유지
2. 실주문 기능은 기본 OFF (모든 자동주문은 비활성 상태 유지)
3. 모든 수집/분석/추천/승인 데이터에 `trace_id` 유지
4. 신규/변경 DB 테이블 및 컬럼은 **TABLE COMMENT / COLUMN COMMENT 필수**
5. JPA 엔티티 / 주요 서비스에 한국어 주석 작성
6. 기능 단위 커밋 + push
7. 한 번에 전체 구현 금지 (차수별 지시 범위만 수행)
8. Mock 데이터는 테스트용으로만 허용하고, 운영 화면에는 실데이터 여부를 구분 가능하게 표시

---

## 2. 현재 문제 정의 (수정 대상 명확화)

### A. 배포 검증 실패 가능성 (핵심)
현재 배포 상태는 “화면 표시”만으로 성공 판단된 것으로 보이며, 다음 검증이 누락되었을 가능성이 높다.

- 서버 로그 기반 오류 확인 미흡
- 브라우저 네트워크 응답(JSON) 점검 미흡
- DB 적재 데이터의 provider 확인 미흡 (MOCK vs TOSS/KIWOOM)
- Feature Toggle / 환경변수 / profile 확인 미흡
- 스케줄러 동작 여부 및 수집 실패율 확인 미흡

### B. 뉴스 썸네일 미표시
뉴스 카드 썸네일이 여전히 텍스트만 표시됨.
이는 다음 중 하나가 원인일 가능성이 높다.

- RSS/기사 HTML에서 이미지 추출 실패
- OG(OpenGraph) 이미지 파싱 미구현 또는 실패 처리 미흡
- 이미지 URL 저장 컬럼 없음 또는 미사용
- 프론트 렌더링 시 `thumbnail_url` 대신 placeholder 텍스트 사용
- CORS / mixed-content / CSP 정책으로 이미지 차단

### C. 실데이터 미반영 (Mock 의심)
현재 주식/자원 데이터가 실 API가 아닌 mock 데이터일 가능성이 높다.

- Provider 선택값이 `mock`으로 고정
- API 키 미설정으로 fallback 동작
- 수집 실패 시 mock 반환 후 UI에 구분 없이 표시
- DB에 `provider_name='MOCK'`만 저장되는 상태

### D. 전략패널/상세 분석 품질 문제
- 국가별/대표주 위주 반복 노출
- 핵심분야 선택 미반영
- 종목별 호재/악재 확률이 50%±5% 수준에 고정
- 상세 분석이 실질 근거 없이 숫자만 표시
- 동일 정보 중복 표시가 심함
...
### 전체 구조 개요
- **뉴스 수집 계층**: 기사 수집 + 본문 추출 + 썸네일 추출 + 번역/정규화
- **시장데이터 수집 계층**: TossMCP / 키움 REST / Mock(테스트용)
- **종목 유니버스 엔진**: 핵심분야 + 국가 + 전략별 후보군 구성
- **뉴스-종목 매핑 엔진**: 별칭/티커/테마/이벤트 기반 연결
- **시그널 엔진**: RULE_V1 기반 확률/추천/리스크 계산
- **RAG 보조 엔진**: 규칙 결과를 설명/근거 요약 보강
- **AI 비서 UI**: 실시간 상태, 종목별 추천근거, 전략탭, 관리자 진단
- **주문 파이프라인(준비 단계)**: 추천 → 승인 → 모의주문 (실주문 OFF)

---

## 4. 반드시 해결해야 할 기능 요구사항 (핵심)

### 4.1 배포 오류 검증/진단 체계 (신규 필수)
배포 후 “정상 여부”를 UI가 아니라 **로그/DB/API/스케줄러 기준**으로 판단할 수 있어야 한다.

#### 필수 진단 체크리스트 API/화면
- 수집 성공률 / 실패율 / 지연시간
- provider별 최근 수집 결과 (TOSS/KIWOOM/MOCK)
- 최근 10건 예외 로그 요약
- feature toggle 상태
- 실데이터 여부 점검 결과 (mock 사용 여부)
- 스케줄러 실행/미실행 상태
- trace_id 기반 상세 추적

#### 배포 검증 체크리스트 (화면 + DB + API)
- `/actuator/health` 정상
- `market_provider_job` 최근 실행 성공
- `market_quote_snapshot`에 실 provider 데이터 적재
- `news_article` + `news_thumbnail` 또는 `thumbnail_url` 적재 확인
- 전략패널 API 응답에 `provider_name` / `trace_id` 포함 확인

---

### 4.2 뉴스 썸네일 수집/저장/표시 구조 개선 (반드시 수정)
뉴스 썸네일은 텍스트가 아니라 실제 이미지로 표시되어야 한다.
...
4. 본문 첫 번째 유효 이미지
5. 없으면 기본 placeholder 이미지 URL (텍스트 금지)

#### 저장 구조 (예시)
- `news_article.thumbnail_url`
- `news_article.thumbnail_source` (RSS/OG/TWITTER/BODY/DEFAULT)
- `news_article.thumbnail_status` (SUCCESS/EMPTY/FAILED)
- `news_article.thumbnail_width`, `thumbnail_height` (선택)

#### 표시 규칙
- 텍스트 placeholder 금지
- 이미지 실패 시만 기본 이미지 표시
- `thumbnail_status`가 FAILED이면 관리자 진단에 누적
- 동일 기사 재수집 시 썸네일 재파싱 가능하도록 상태 관리

---

### 4.3 실 API 기반 시장데이터 수집 (반드시 실데이터 우선)
Mock/모의가격이 아닌 실제 API 데이터를 화면에 반영해야 한다.

#### Provider 추상화 (필수)
- `MarketDataProvider` 인터페이스
- `TossMcpMarketDataProvider`
- `KiwoomRestMarketDataProvider`
- `MockMarketDataProvider` (테스트용, 운영 기본값 금지)

#### 운영 강제 모드 (필수)
- `market.provider.active=toss|kiwoom`
- `market.provider.allowMock=false` (운영 기본)
- API 키 없거나 호출 실패 시:
  - 무조건 mock 반환 금지
  - 오류 상태 기록 + 관리자 진단에 노출
  - UI에는 “데이터 미수신/지연” 상태 표시
...
### 4.4 핵심분야 반영 + 중복 종목 제거 + 유니버스 동적 재구성
전략패널이 단순 대표주 나열이 되면 안 된다.
...
#### 유니버스 정책
- 국가별/테마별 후보군 구성
- 시총/거래대금/뉴스 언급량/최근 변동성/사용자 관심 여부 기반 점수화
- 전략별(단타/중기/차트대응/6개월 발굴) 후보군 분리
- 최근 노출 종목 쿨다운 적용 (중복 억제)

#### 중복 억제 규칙
- 동일 종목이 여러 패널에 반복 노출되는 횟수 제한
- 동일 테마 편중도 제한
- 결과 부족 시 fallback 규칙은 적용하되, 중복 억제 로그를 남김

---

### 4.5 뉴스-종목 매핑 + 호재/악재 확률 계산 고도화 (50% 수렴 방지)
현재 50:50 수렴 문제를 해결해야 한다.

#### 뉴스-종목 매핑 로직 (필수)
- 티커/종목명/별칭/브랜드명 사전 매칭
- 한글/영문/약칭 동시 지원
- 테마 키워드 매칭
- 이벤트 유형 분류:
  - 수주
...
#### 확률 계산 RULE_V1 (필수)
다음 입력 피처를 활용하여 `good_news_probability`, `bad_news_probability`, `uncertainty_score` 산출:
- 뉴스량 변화율
- 이벤트 유형 가중치
- 출처 신뢰도
- 기사 중복도 감점
- 시세 반응(5m/15m/1h 가능 범위)
- 거래량 변화율
- 시간정렬 검증 결과
- 번역 지연/수집 지연 패널티
- 데이터 부족 여부
...
#### 중요 규칙 (반드시 준수)
- `good + bad = 1` 강제 금지
- 데이터 부족 시 억지 수치 생성 금지
- “중립 / 데이터 부족 / 재분석 대기” 상태를 반환할 수 있어야 함
- 상세보기에는 숫자만 아니라 **근거(뉴스/가격/거래량/리스크)**를 분리 표시

---

### 4.6 전략 엔진 분리 + 차트/뉴스 복합 전략 반영
사용자 요구 매매 방식(차트 + 뉴스 결합)을 로직에 반영해야 한다.

#### 전략 분리
1. 최근 뉴스 검색 (단타/매매)
2. 시장 동향 (중기/일 매매)
3. 차트 분석 (장기 전 대응 전략, 평단가 낮추기형 대응)
4. 6개월 후 뜰만한 주식 발굴
...
- 일정 구간 지속 매수 발생
  - **거래량 동일시 호재로 단정 금지**
- 목표 달성 금액/수익률 기준
  - 투자금 기준 분할매수/분할매도 비율 계산
  - 익절/손절 발생 후 재분석 전까지 매수 금지(BUY_LOCK)

#### 리스크/자금관리 패널 (의미 있게 재구성)
- 포트폴리오 분산도 경고 (몰빵 방지)
- 전략별 차단 건수/차단 사유
- BUY_LOCK 종목 목록
- 데이터 품질 저하 종목
...
### 4.7 RAG 기반 AI 주식비서 (경량 모델, 서버부하 최소화)
RAG는 규칙 엔진을 대체하지 않고 **설명/요약/근거 보강** 역할로 사용한다.

#### 역할
- 종목 상세 분석 설명문 생성
- 추천 이유/주의점 요약
- 뉴스/시세/리스크 근거를 사람이 읽기 좋은 문장으로 정리
- 관리자 trace 조회 가독성 개선

#### 필수 조건
- 경량 모델 또는 비용 통제 가능한 구성
- timeout / fallback / circuit breaker
- RAG 실패 시 규칙기반 응답 유지
- `model_version`, `prompt_version`, `latency_ms`, `fallback_used` 기록
...
### 4.8 AI 비서 화면 / 관리자 진단 화면 고도화
#### AI 비서 화면(`/assistant`)
- 상단 상태바: 시장상태 / provider 상태 / 수집상태 / 자동분석 ON/OFF / 실주문 OFF 표시
- 관심종목 패널:
  - 현재가
  - 등락률(%)
  - 거래량
  - 뉴스 개수(1h/24h)
  - 추천액션(BUY/WATCH/HOLD/SELL)
  - 신뢰도/불확실성
  - 리스크 배지
- 전략 탭:
  - 단타
...
- 종목 상세:
  ...
  - 진입/익절/손절 참고값
  - 재분석 조건
- (선택) AI 비서 Q&A 영역 (trace 기반 근거 표시)

#### 관리자 진단 패널
- 수집 품질(성공률/지연/결측/중복)
- provider 감사 (TOSS/KIWOOM/MOCK 사용 내역)
- 썸네일 수집 성공률/실패율
- 유니버스 다양성/편중도
- 뉴스-종목 매핑 품질
- 전략 액션 분포/차단 사유 분포
- RAG 성공률/지연/timeout/fallback
- trace 상세 조회

---

### 4.9 자동매매 확장 준비 (실주문 OFF 유지)
최종 목표는 자동화 가능 구조지만, 현재 단계에서는 **관리자 승인 기반 모의주문 파이프라인**까지만 준비한다.

#### 주문 파이프라인 상태 흐름
`ANALYZE -> RECOMMEND -> APPROVE -> ORDER_REQUESTED -> ORDER_EXECUTED(PAPER)`

#### 필수 조건
- 실주문 기능 기본 OFF (`LIVE_TRADE=false`)
- 관리자 승인 없이는 주문 요청 금지
- 추천/승인/주문결과 모두 trace_id로 재현 가능
- 승인/반려 사유 기록
...
### 공통 원칙
- 모든 신규/변경 테이블에 `COMMENT ON TABLE`
- 모든 신규/변경 컬럼에 `COMMENT ON COLUMN`
- 주요 테이블에 `trace_id`, `created_at`, `updated_at` 고려
- 상태값 컬럼은 enum/코드 정의 문서화
- 감사 로그 테이블은 JSON 근거 필드 허용

### 최소 필요 테이블(신규/보강)
- `market_quote_snapshot` (실시간 시세 스냅샷)
- `market_price_bar` (분봉/시계열 바 데이터)
- `market_provider_job` (수집 작업 실행/실패/지연 감사)
- `asset_universe` (전략/테마/국가 기반 종목 유니버스)
- `news_article` (썸네일/상태 포함 보강)
- `news_asset_link` (뉴스-종목 매핑 근거)
- `trading_signal` (추천 결과)
- `signal_audit_log` (근거/확률/RAG/품질 감사)
- `trade_approval` / `paper_trade_order` (추후 단계)

---

## 6. API 응답 원칙 (일관성 유지)
모든 관리자/분석 API 응답은 다음 구조를 기본으로 사용한다.
...
```json
{
  ...
  "meta": {
    "trace_id": "string",
    "provider_name": "TOSS|KIWOOM|MOCK",
