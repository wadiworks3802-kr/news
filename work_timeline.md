...
- [T238] 종목 유니버스 재구성 4차 착수: "핵심분야 선택했는데 반영 안되는 느낌"과 전략패널 동일 종목 반복 노출 문제를 재현/진단하기 위해 `asset_universe` 스키마·유니버스 재구성 점수·전략패널 dedup/cooldown·관리자 다양성 진단 확장 범위를 확정.
- [T239] `asset_universe` 스키마 보강(4차): `V15__asset_universe_scope_and_exposure_fields.sql` 추가로 `country_code`, `strategy_scope`, `last_panel_exposed_at`, `panel_exposure_count_24h` 컬럼/인덱스 및 `COMMENT ON COLUMN`을 적용하고 `AssetUniverseEntity`에 한글 `@Comment` 필드를 반영.
- [T240] 유니버스 재구성 로직 개선(4차): `UniverseRebuildService`에 국가/테마/전략 범위(`strategy_scope`) 저장, 핵심분야(theme) 후보군 점수(유동성/변동성/뉴스언급량/활성여부) 반영, 최근 패널 노출/쿨다운 카운터 기반 진단 집계를 추가.
- [T241] 전략패널 중복 억제 강화(4차): `TradingSignalEngineService`에 전략 범위 필터, 동일 종목/패밀리/테마 편중 제한, 최근 노출 종목 쿨다운, 노출 카운터 증가 로직(`lastPanelExposedAt`, `panelExposureCount24h`)을 적용하고 반복 노출/쿨다운 필터 통계를 WARN 로그와 메타로 남기도록 확장.
- [T242] 전략패널 API 연결/진단 메타 확장(4차): `TradingSignalViewDto`, `InsightController`에 `country_code`, `strategy_scope`, `panel_exposure_count_24h`, `panel_cooldown_active` 및 `theme_selection_reflected`, `requested_theme_code`, 분포/중복/쿨다운 카운터 메타를 추가해 핵심분야 선택 반영 여부를 API 응답에서 직접 확인 가능하게 보강.
- [T243] 관리자 유니버스 다양성 진단 보강(4차): `AdminDiagnosticsService`에 `assets_by_strategy_scope`, `active_panel_cooldown_assets`, `repeated_panel_exposure_assets`, 경고 목록을 추가하여 편중도/쿨다운 상태를 운영 진단 API에서 확인 가능하게 확장.
- [T244] 재현 버그 수정(4차): `InsightController`의 `scalp/swing/discovery` 메타 생성 시 `Map.of(... theme_code=null ...)`로 인한 `NullPointerException`(theme 미지정=ALL 케이스 500)을 재현하고, `normalizePanelMetaToken(..., \"ALL\")` 적용으로 `theme_code`를 non-null로 보정해 200 응답으로 복구.
- [T245] 로컬 검증/증거 수집(4차): `:api` 로컬 기동 후 전략패널/유니버스 다양성/시장진단 API에서 `trace_id`, `provider_name(mock 경고)`, `theme_selection_reflected`, `strategy_scope_distribution`, `panel_cooldown_active_count`를 확인하고, H2(`api/.data/gnd`)에서 `asset_universe` 신규 컬럼/분포/쿨다운 카운터 및 COMMENT 반영 여부를 조회해 증거를 수집.
- [T246] 5차 범위 재정렬 착수: 뉴스-종목 매핑/확률 RULE_V1 요구사항(50% 수렴 금지, 데이터 부족 상태값 분리, 상세 근거 분리 반환)을 기존 구현 기준으로 재점검하고, 미흡한 상세 응답/파싱 경로 보강 범위를 확정.
- [T247] RULE_V1 데이터부족 확률 보강(5차): `ScalpNewsSignalService`의 데이터 부족/매핑 부족(no matched/insufficient sample) 구간에서 `good=0.5`, `bad=0.5` 고정값을 제거하고, 희소 근거 질량 기반 저강도 확률(`sparseGood/sparseBad`) 계산 + 패널티 적용으로 50:50 수렴을 완화.
- [T248] 상태값 분리 강화(5차): `ScalpNewsSignalService`에 `analysis_state`(`INSUFFICIENT_DATA`/`REANALYSIS_PENDING`/`NEUTRAL`/`EVIDENCE_READY`) 계산 로직을 추가하고 `data_state`와 함께 `probability_reason_breakdown_json`에 저장되도록 확장.
- [T249] 상세/패널 응답 근거 필드 확장(5차): `TradingSignalEngineService`, `TradingSignalViewDto`, `SignalDetailDto`에 `analysis_state`, `data_state`, `news_confidence`, `volume_evidence` 노출을 추가하고, 상세 응답에서 뉴스/가격/거래량/리스크 근거를 분리 반환하도록 보강.
